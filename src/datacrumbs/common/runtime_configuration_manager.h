...
class RuntimeConfigurationManager {
 public:
  ...
  /// Registers a record kind a plugin will emit; returns the id the writer labels it with.
  /// Ids sit above the built-in telemetry range, so a plugin cannot collide with a probe or a
  /// configured source.
  uint64_t register_plugin_event(const char* category, const char* name, const char* type);

  /// What attach did, so the trace can say whether a missing layer was quiet or never watched.
  /// Set once when attachment finishes; read when the trace closes.
  std::atomic<unsigned long long> probes_requested{0};
  std::atomic<unsigned long long> probes_attached{0};
  std::atomic<unsigned long long> probes_failed{0};
  std::atomic<unsigned long long> hot_requested{0};
  std::atomic<unsigned long long> hot_attached{0};
