...
// SPI a datacrumbs plugin compiles against. A plugin is a shared object listed in
// DATACRUMBS_PLUGINS that exports `datacrumbs_plugin_register`; the server dlopens it and calls
// that symbol with a PluginApi bound to the server's own registries. The plugin registers only
// through the passed api, so it never depends on symbol interposition with the executable.
...
// Unlike an enricher, it can pull data with no probe or counter behind it, for example the DPA
// telemetry API. Running it in the server keeps it node-scoped and alive across configurations.
