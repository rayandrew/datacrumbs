...
// One self-contained gzip member; the .pfw is these concatenated, so a reader streams member by
// member.
...
// dftracer host key: md5(hostname), even-indexed digest bytes in %02x (matches df_logger.h
// get_hash).
