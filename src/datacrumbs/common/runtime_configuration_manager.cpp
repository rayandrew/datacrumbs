...
// System-wide interval telemetry, driven only by env vars, no probe file.
// DATACRUMBS_TELEMETRY_INTERVAL_MS sets the period. DATACRUMBS_UNCORE_EVENTS is a comma list of
// libpfm4 uncore event names for one perf track. DATACRUMBS_NIC_DEVICES is a comma list of
// "<ibdev>[:port]", port default 1, each producing an IB sysfs track plus an RDMA hw_counters
// track.
void RuntimeConfigurationManager::derive_telemetry_sources() {
  // Each name is a counter track keyed by (pid, name). pid is 0 here, so names must be unique
  // within a node.
  // The node id lives in args.hhash, per the dftracer record convention.
  ...
  while (std::getline(ss, token, ',')) {
    ...
    // Firmware RDMA health counters: retransmits, timeouts, errors invisible to perf and uprobes.
    // A counter missing on this HCA is a missing sysfs file, skipped per tick, so the list stays
    // portable across HCAs.
    ...
    // Same counters as above, scoped to the bound QPs so other traffic on the NIC does not pollute
    // them. Needs per-port auto mode. Without it, the dump returns nothing and the source stays
    // silent.
    TelemetrySource qp;
    qp.cat = "nic_qp";
  ...
  // mlx5 vport counters via SIOCETHTOOL, keyed by NETDEV, not ibdev. The only source that sees
  // DevX/DOCA RDMA bytes the IB port counters miss. `path` holds the ethtool stat name.
  // Per-process hardware counters on a fixed interval, separate from DATACRUMBS_HW_COUNTERS, which
  // feeds the BPF cpu-scope arrays attributable to a core, not to the traced program.
  ...
  // Utilisation from procfs: node busy-ness, memory resident, thread preemption, none of which a
  // PMU reports. BlueField-3 exposes only armv8_pmuv3_0 and no uncore PMU, so procfs is the only
  // node-level source there. `path` is the field name; the sampler resolves it against /proc.
  // Counters are cumulative, so the existing delta-per-tick handling applies unchanged.
  {
    TelemetrySource node;
    node.cat = "proc_node";
    node.trace_event_type = "node";
    node.name = "node";
    // /proc/stat cpu jiffies and scalar counters.
    ...
    // Process scope, summed over traced tgids like the task PMU source: CPU used, memory resident,
    // fault rate. Reported alongside node figures so a busy process on an idle node is
...
// Telemetry sources get event ids in a high range so they never collide with probe ids.
// Call after load_runtime_probe_file, which populates category_map.
