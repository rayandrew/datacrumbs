...
int ethtool_stat_count(int fd, const char* ifname) {
  ...
  // hdr.data is a kernel zero-length array from linux/ethtool.h. buf[0] holds the real storage.
...
// Pulls every stat for one source in a single ioctl. Per-counter reads index into this.
...
namespace {
// Minimal netlink attribute walk. RDMA nldev replies are nested tables of type, length, and
// payload.
struct NlAttrView {
...
// Resolves an ibdev name to its nldev index via a RDMA_NLDEV_CMD_GET dump.
// /sys/class/infiniband/<dev>/index is missing on some kernels, BlueField DOCA kernels
// included, so netlink is the portable way.
...
// One STAT_GET dump for this port. Sums each hw counter across the bound counter sets.
...
// Traced tgids come from the BPF pid_map. A process that appears mid-run gets counters from then
// on.
...
// utime and stime are fields 14 and 15 of /proc/<pid>/stat, num_threads is field 20.
// Resident size comes from status instead, because stat reports pages and status reports kB.
...
bool ProcTelemetrySampler::read_raw(std::size_t src, std::size_t ctr, unsigned long long* out) {
  ...
  // Process scope, summed across the traced tgids like the task PMU sampler does.
  // One value per counter for the whole workload, not a source per pid that would
  // churn as processes come and go.
