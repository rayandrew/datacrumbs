...
// Minimal bpftime raw-API decls, kept in sync with the bpftime runtime's own headers, to avoid
// pulling them and boost into this build. bpf_map_attr and the enum layout must match
// exactly; they are passed by value across the ABI.
namespace bpftime {
struct bpf_map_attr {
  int type = 0;
...
int bpftime_hot_init(struct bpf_object* obj) {
  ...
  // Forward each DATACRUMBS_* env var to the name bpftime reads, before it initializes.
  // Do not overwrite an explicit upstream value.
  static const struct {
    const char* ours;
    const char* theirs;
  } kForward[] = {
  ...
  // An unprivileged workload agent must write this shm to register its uprobes, but a root
  // server creates it 0644. Widen permissions for both the new-file and pre-existing case.
