...
//
// Probe arguments are raw memory, so a captured char* is often not valid UTF-8.
// The escaper must turn any byte sequence into valid JSON.
...
int main() {
  // A lone 0x96 is not valid UTF-8 and must not reach the output raw.
  ...
  // Real text must survive unchanged.
  ...
  // A probe copies a fixed number of bytes, which can cut a multi-byte character in half.
  ...
  // Test every byte value alone, then every two-byte pair. This covers the lead-plus-continuation
  // decisions exhaustively instead of only at a few hand-picked points.
  for (int b = 0; b < 256; ++b) {
